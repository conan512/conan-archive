...
TIME_BUDGET = int(os.environ.get("TIME_BUDGET", "0"))  # seconds, 0 = unlimited
...
def main():
    posts = list(store.load()[1].values())
    ...
    # self-heal: if the manifest is missing/stale, rebuild it by scanning disk.
    # Filenames ARE the url hashes, so the mapping is fully recoverable.
    have = set()
    for sub in os.listdir(MEDIA) if os.path.isdir(MEDIA) else []:
        d2 = os.path.join(MEDIA, sub)
        if os.path.isdir(d2):
    ...
    for u in urls:
        ...
        if k in have and k not in man:
            ...
            rebuilt += 1
    if rebuilt:
        print(f"recovered {rebuilt} images from disk scan")
    for k in [k for k in man if k not in have]:
        del man[k]
        rebuilt = -1
    if rebuilt == -1:
        print(f"pruned manifest entries whose files are gone -> {len(man)} valid")
    if rebuilt:
        save(man, failed)   # persist recovery immediately, before any download
    ...
    with cf.ThreadPoolExecutor(max_workers=WORKERS) as ex:
        futs = [ex.submit(guarded, u) for u in todo]
        for _ in cf.as_completed(futs):
            done += 1
            if TIME_BUDGET and time.time() - t0 > TIME_BUDGET and not stop.is_set():
                stop.set()
                print(f"  ⏱ time budget {TIME_BUDGET}s reached — "
                      f"stopping early, rest continues next run", flush=True)
                save(man, failed)
            if done % 250 == 0:
                el = time.time() - t0
