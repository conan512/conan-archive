...
def main():
    ...
    while pages < MAX_PAGES:
        ...
        if pages % 25 == 0:
            n, ns = store.save(meta or _meta_old, posts,
                               time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()))
            print(f"  checkpoint: {n} posts in {ns} shards")
        if before <= 1:
            break
        time.sleep(0.6)
    ...
    n, ns = store.save(meta or _meta_old, posts,
                       time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()))
    print(f"saved {n} posts in {ns} shards -> data/posts/")
